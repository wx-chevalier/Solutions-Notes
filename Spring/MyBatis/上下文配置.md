...
Mybatis 的内置日志工厂提供日志功能，内置日志工厂将日志交给以下其中一种工具作代理：SLF4J, Apache Commons Logging, Log4j 2,
Log4j, JDK logging。MyBatis 内置日志工厂基于运行时自省机制选择合适的日志工具，它会使用第一个查找得到的工具（按上文列举的顺序查找）；如果一个都未找到，日志功能就会被禁用。
